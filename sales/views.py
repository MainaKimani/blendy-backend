...
class SalesAgentProfileViewSet(OrganizationBaseViewSet):
    ...
    def get_permissions(self):
        if self.action == 'list' or self.action == 'retrieve':
            return [IsAuthenticated(), IsOrganizationUser(), HasUserPermission('sales.view_salesagentprofile')]
        elif self.action == 'create':
            return [IsAuthenticated(), IsOrganizationUser(), HasUserPermission('sales.add_salesagentprofile')]
        elif self.action == 'update' or self.action == 'partial_update':
            return [IsAuthenticated(), IsOrganizationUser(), HasUserPermission('sales.change_salesagentprofile')]
        elif self.action == 'destroy':
            return [IsAuthenticated(), IsOrganizationUser(), HasUserPermission('sales.delete_salesagentprofile')]
...
class IssueStockToAgentView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not all([product_variation_id, agent_id, location_id, quantity > 0]):
            return Response({"error": "Missing required fields or invalid quantity."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if inventory_item.available_quantity < quantity:
                return Response({"error": "Not enough stock available in the warehouse."}, status=status.HTTP_400_BAD_REQUEST)

            agent = CustomUser.objects.get(id=agent_id, organization=request.organization)
            product_variation = ProductVariation.objects.get(id=product_variation_id, organization=request.organization)
            ...
            return Response({"success": f"Successfully issued {quantity} of {product_variation.name} to {agent.username}."}, status=status.HTTP_200_OK)

        except InventoryItem.DoesNotExist:
            return Response({"error": "Inventory item not found at this location."}, status=status.HTTP_404_NOT_FOUND)
...
class SaleViewSet(OrganizationBaseViewSet):
    ...
    def perform_update(self, serializer):
        serializer.save(updated_by=self.request.user, organization=self.request.organization)
