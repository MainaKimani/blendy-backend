...
def get_backoff_delay_minutes(retry_count: int, base_minutes: int = 5, max_minutes: int = 12 * 60) -> int:
...
def schedule_next_retry(payment) -> None:
    ...
    payment.save(update_fields=["retry_count", "last_retry_at", "next_retry_at", "updated_at"])
